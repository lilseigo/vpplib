        ...
        if type(timestamp) == int:
            ...
            if pd.isna(next(iter(self.timeseries.iloc[timestamp]))) == False:
                ...
                if self.is_running:
                    ...
                    if self.heat_pump_type == "Ground":
                        temp = self.user_profile.mean_ground_temp_quarter_hours.ground_temperature.iloc[
                            timestamp]
            ...
            if pd.isna(next(iter(self.timeseries.loc[timestamp]))) == False:
                ...
                if self.is_running:
                    ...
                    if self.heat_pump_type == "Ground":
                        temp = self.user_profile.mean_ground_temp_quarter_hours.ground_temperature.loc[
                            timestamp]
                    cop = self.get_current_cop(temp)
                    thermal_energy_output = el_demand * cop
                else:
            ...
            if pd.isna(next(iter(self.timeseries.loc[str(timestamp)]))) == False:
                ...
                thermal_energy_output, cop, el_demand = self.timeseries.loc[str(
                    timestamp)]
                ...
                if self.is_running:
                    el_demand = self.el_power
                    if self.heat_pump_type == "Air":
                        temp = self.user_profile.mean_temp_quarter_hours.temperature.loc[str(
                            timestamp)]
                    if self.heat_pump_type == "Ground":
                        temp = self.user_profile.mean_ground_temp_quarter_hours.ground_temperature.loc[str(
                            timestamp)]
                    cop = self.get_current_cop(temp)
                    thermal_energy_output = el_demand * cop
                else:
